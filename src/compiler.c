...
Parser parser;
Compiler* current = NULL;  // if compiler is multi-threaded, this can't be global
...
static void patchJump(int offset) {
  ...
  if (jump > UINT16_MAX) { // if more than 2 bytes needed, create OP_LONG_JUMP
...
static void endScope() {
  ...
  while (current->localCount > 0 && current->locals[current->localCount - 1].depth > current->scopeDepth) {
    emitByte(OP_POP);  // PERF: add op OP_POPN to cleanup stack quickly
    current->localCount--;
  }
}
...
static bool identifiersEqual(Token* a, Token* b) {
  ...
  return memcmp(a->start, b->start, a->length) == 0;  // PERF: could use crinha Strings to compare hashes
}
...
static void binary(bool canAssign) {
  ...
  switch (operatorType) {
    ...
    case TOKEN_QUOT: emitByte(OP_MODULUS); break;
    default:
      return;
  }
}
...
ParseRule rules[] = {
    ...
    [TOKEN_QUOT] = {NULL, binary, PREC_FACTOR},
...
bool compile(const char* source, Chunk* chunk) {  // single pass, could be multiple pass to stretch performance
  initScanner(source);
  Compiler compiler;
